...
import { BicepsFlexed, ChartNoAxesCombined, ChevronDown, Code, House, LogIn, LogOut, NotebookPen, Shapes, TabletSmartphone, Trophy, VenetianMask } from 'lucide-react';
const Navbar = () => {
        ...
        {/* image */}
            ...
            <nav>
                <ul className='flex flex-row gap-6'>
                                ...
                                {0?(
                                 ...
                                 subLinks.length>0 ? (
                                        subLinks.map( (subLink, index) => (
                                                <Link to={`catalog/${subLink.name.split(" ").join("-").toLowerCase()}`} key={index} className='rounded-lg bg-transparent py-2 pl-2 hover:bg-richblack-50 hover:bg-yellow hover:text-black'>
                                                    <p>{subLink.name}</p>
                                                </Link>
                                            ) )
                    ...
                    <li><Link to={"/practice"}><p className={`${matchRoute("/practice")?"text-yellow":""} flex gap-1 items-center`}><Trophy  size={20}/>Practice</p></Link></li>
                    <li><Link to={"/notes"}><p className={`${matchRoute("/notes")?"text-yellow":""} flex gap-1 items-center`}><NotebookPen size={20}/>Notes</p></Link></li>
                    <li><Link to={"/jobs"}><p className={`${matchRoute("/jobs")?"text-yellow":""} flex gap-1 items-center`}><ChartNoAxesCombined  size={20}/>Jobs</p></Link></li>
                </ul>
            </nav>
